...
(https://archive.ics.uci.edu/ml/machine-learning-databases/00222/) can be 
...
control <- trainControl(method = "cv", number = 10, classProbs = TRUE, verboseIter = TRUE)
```
...
# develop a model
set.seed(7)
glm_model <- train(subscribed ~ . - duration_sec, data=training, method="glm", na.action=na.omit, metric="Kappa")
```
...
```{r}
glm_predict_full <- predict(glm_model, newdata=validation)
```


Here is a confusion matrix, with the results expressing accuracy, sensitivity, 
and specificity. The metric parameter Kappa will be used because it's better 
than F1 for uneven results.
